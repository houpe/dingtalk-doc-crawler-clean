...
> **⚠️ 架构变更提醒（2026-07-15）**：`help.beta.ztocc.com` 现在前面有一层 Node 钉钉鉴权服务（pm2 进程 `docs-auth`，监听 127.0.0.1:4000），nginx 全量 proxy_pass 到它。本文档的 tar+scp 部署命令**只更新静态文档内容**（`/www/wwwroot/help.beta.ztocc.com`），Node 服务通过 `DOCS_DIST_DIR` 实时读取该目录，所以更新文档**不需要重启 Node、不影响鉴权**。但如果你要改鉴权逻辑本身，请阅读 **[钉钉鉴权部署说明.md](./钉钉鉴权部署说明.md)**。
...
- 第一步“文档生成”：默认增量同步钉钉中新建或变更的文档、图片，并删除本地已在钉钉消失的文档，再过滤和优化为 `output_optimized`
...
钉钉抓取会在 `output/.dws-crawl-state.json` 记录每份文档的 `nodeId`、更新时间、保存路径和图片完成状态。日常使用“增量抓取钉钉文档”或第一步的一键流程即可：它仍会扫描钉钉目录以识别新增、删除、改名和移动，但不会重复读取未变的正文或下载未变图片。管理台日志会逐项显示变更的文章和行级增删摘要；完整结果保存在 `output/.dws-crawl-last-report.json`，其中含文章路径、更新时间、变更行数和最多 12 条增删行预览。

名称包含“系统更新日志”的文档会额外作为内部核对来源：抓取器解析其中的功能名称和优化点，用功能关键词匹配操作手册标题和正文，生成 `output/.dws-changelog-review.json`。该报告列出“待核对”的文章及对应更新项；更新日志本身仍会在后续内容过滤阶段排除，不会发布到站点。日志只用于定位，不能代替完整操作步骤，因此不会自动改写手册。首次初始化、状态文件异常，或需要彻底以钉钉为准时，使用“全量重抓钉钉文档”；该操作会先清空整个 `output/`。
